...
# HuggingFace is the sole LLM backend for this project — no Gemini/OpenAI
LLM_BACKEND  = "huggingface"

# Qwen2.5-3B-Instruct: native transformers support, no remote code,
HF_MODEL_ID  = "Qwen/Qwen2.5-3B-Instruct"

HF_TOKEN     = os.environ.get("HF_TOKEN", "")     # not required for Qwen (ungated)
...
COUNTRY_RISK_BASELINE = {
    "Russia": 55, "China": 45, "North Korea": 60, "Iran": 50, "Syria": 58,
    "Myanmar": 48, "France": 28, "Germany": 8, "UK": 7, "Israel": 20,
    "South Africa": 14, "Austria": 10, "Spain": 9, "Singapore": 5,
    "India": 6, "USA": 4, "Canada": 4, "Australia": 4, "Japan": 7,
    "South Korea": 9, "Taiwan": 25, "Poland": 12, "Romania": 12,
    "Bulgaria": 13, "Belgium": 9, "Sweden": 6, "Norway": 5,
}
...
ALIAS_MAP: dict = {}   # add spelling aliases here if needed, e.g.
...
_COLUMN_ALIASES = {
    "supplier":       {"supplier", "suppliers", "tier1_supplier",
...
for _, row in owner_df.iterrows():
    ...
    # OPTIONAL column — if your ownership.csv has a `financial_distress_score`
    fin = row.get("financial_distress_score", None)
    try:
...
for _key, _owners in OWNERSHIP_REGISTRY.items():
    _vals = [o["financial_distress_score"] for o in _owners
             if o.get("financial_distress_score") is not None]
...
def financial_distress_score(supplier_name: str) -> float:
    ...
    Reads the optional `financial_distress_score` column from ownership.csv
    if present; defaults to 0 for any supplier with no such data (i.e. this
    term simply doesn't contribute until you add that column)."""
...
print(f"✅ Supply chain graph: {len(NEO4J_GRAPH)} edges")
...
# ── CELL 9: CREAO agent config files (OPTIONAL — purely cosmetic) ──
# IMPORTANT: the `marketplace.json` / `plugin.json` you uploaded to this
import yaml, json as _json

CREAO_CONFIG    = None
CREAO_MANIFEST  = None
CREAO_DASHBOARD = None
CREAO_SKILL     = None
...
if missing:
    try:
        ...
        print("📂 Optional CREAO agent files")
        print("Upload any or all of the following in ONE upload dialog:")
        for f in required_files:
            print(f"   • {f}")
        print("\nOr press Cancel to use built-in defaults.\n")
...
CREAO_DASHBOARD = load_if_exists("dashboard.json", _json.load)
...
if CREAO_DASHBOARD:
    print(f"📊 dashboard.json loaded ({len(CREAO_DASHBOARD)} keys)")
...
def build_supplier_rollup() -> list:
    ...
    for name in sorted(all_supplier_names):
        edges      = [e for e in NEO4J_GRAPH if e["tier1"] == name or e["tier2"] == name]
...
def _batch_call(llm_fn, scenario: str, batch: list) -> dict:
    ...
    Atmanirbhar Bharat) or allied-nation suppliers, and give a one-word
    component_category (e.g. "electronics", "propulsion", "optics").
...
def fill_alternates(analysis: dict, enriched_suppliers: list) -> list:
    ...
    for s in enriched_suppliers:
        if s["name"] not in covered:
            ...
            for a in alts:
                if a["replaces"] == s["name"] and "component_category" not in a:
                    a["component_category"] = _indian_alternate(
                        s["component"], s["name"]).get("component_category", "—")
    return alts
...
# ── CELL 19: Plotly dashboard (run this AFTER the script above) ───
# This is your "second cell" — it only reads the `analysis` /
import plotly.graph_objects as go
import plotly.io as pio
from plotly.subplots import make_subplots
from IPython.display import display, HTML

# FIX for "no bar graphs" — Colab sometimes fails to auto-detect the
...
def render_dashboard(analysis: dict, ownership_data: list):
    ...
    # Apply the same boosted scores back onto the full supplier list so the
    ...
    top12 = crisis_alts[:12]
    if top12:
        fig.add_trace(_table(
            ["Replaces", "Alternate Supplier", "Country", "Category",
             "Components Supplied", "Reason"],
            [
