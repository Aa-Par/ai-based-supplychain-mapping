...
Original file is located at
    https://colab.research.google.com/drive/1KxnXazZ8wveTKi8N7HLZHTBW-YKcDnZF
"""

# ── CELL 1: Install ───────────────────────────────────────────
!pip install -q transformers accelerate torch plotly pandas requests pyyaml
...
# no custom Cache subclasses → immune to the whole class of bugs that
# broke Phi-3-mini. Drop to "Qwen/Qwen2.5-1.5B-Instruct" if you are on
# CPU-only Colab and want faster (but slightly less sharp) output.
...
COUNTRY_ALIASES = {
    ...
    # or in LLM-generated scenario text. Every comparison in this script
    # goes through normalize_country(), so this is the ONLY place a new
    # spelling variant ever needs to be added.
}
...
def load_ownership_csv(path_hint="/content/ownership.csv") -> pd.DataFrame:
    ...
    expected = {"tier1_supplier", "owner", "owner_type", "owner_country",
...
for _, row in owner_df.iterrows():
    key = normalize_name(row.get("tier1_supplier"))
    if not key:
        continue
    ...
    # column (0–10 scale), it feeds the "Financial Distress Score" term of
    # the risk formula directly. If the column doesn't exist, this is None
    # for every row and the term contributes 0 (see financial_distress_score()).
...
# (vehicle, component) pair. Feeds the "Concentration Penalty" (0–5) term
# of the risk formula: a count of 1 means a single point of failure
# (sole-sourced) for that vehicle/component combination, anywhere in the
# graph — not specific to any country or crisis scenario.
...
# Lets crisis exposure propagate UP the chain: a Tier-1 supplier sitting
# in a perfectly safe country can still be at risk because the Tier-2
# supplier it depends on for a component is located in the crisis zone.
# This is general for ANY supplier / ANY country — built once from the
# graph, not hardcoded to any specific pairing.
...
# project are for an unrelated Claude Code plugin ("ponytail") — NOT
# the CREAO agent's SKILL.md / config.yaml / manifest.json /
# dashboard.json. If you still have the real CREAO files, upload them
# when prompted below; if not, just press Cancel on each dialog and
# the dashboard runs fine on its built-in defaults.
...
def run_ownership_analysis(supplier_names, depth_pct=1.0) -> list:
    ...
    for name in supplier_names:
        ...
        # the input list (e.g. because the LLM repeated it upstream),
        # only analyse it once so the Ownership table never double-prints.
...
#
# WHY THIS CHANGED (v17): the #1 cause of Cells 13b/14/15/16/18/19 never
# finishing was CPU-only greedy generation of a 3B model at up to 2200
# tokens per call, with NO visible progress and NO time limit — it wasn't
# actually frozen, it was just taking 20-60+ minutes per LLM call with no
# feedback. This cell now (a) tells you plainly whether you're on CPU or
# GPU, (b) 4-bit-quantizes the model on GPU for a large speed/VRAM win,
# and pairs with Cell 13b/14's streaming + per-call timeout + batching so
# you always see live progress instead of a silent wait.
...
# model/tokenizer from Cell 13a. Re-run this cell as often as you like
# (e.g. after editing generation params) without touching the weights.
#
# WHY THIS CHANGED (v17): call_hf now (a) streams tokens as they're
# generated so you get a heartbeat print every ~5s instead of dead
# silence, and (b) enforces GENERATION_TIMEOUT_SEC as a hard wall-clock
# cap — if a call runs long, it returns whatever was generated so far
# instead of blocking Cell 14/19 indefinitely. Combined with Cell 14's
# batching (smaller prompts/outputs), this is what actually fixes the
# "never finishes" symptom, rather than just making you wait longer.
...
def build_llm_client():
    if LLM_BACKEND == "huggingface":
        ...
        def call_gemini(system_msg, user_msg, max_new_tokens: int = 900):
        ...
        def call_openai(system_msg, user_msg, max_new_tokens: int = 900):
...
# ── CELL 14: LLM scenario analysis (BATCHED — v17) ─────────────
# WHY THIS CHANGED: v16 asked for the whole portfolio (scenario framing +
# every supplier's score/reason + every alternate) in ONE call with
# max_new_tokens=2200. On a 3B model that single call is what was
# taking 20-60+ minutes with zero visible progress. v17 splits this into
# (1) one small "framing" call for the narrative fields, and (2) several
# small "batch" calls that each score only SUPPLIER_BATCH_SIZE suppliers.
# Each individual call is now short enough to finish in well under
# GENERATION_TIMEOUT_SEC, and you get a printed progress line per batch.
...
def _framing_call(llm_fn, scenario: str, rollup: list) -> dict:
    ...
    return {
        ...
        "immediate_actions": ["Re-run, or switch LLM_BACKEND to 'gemini' / 'openai'."],
    }
...
# ── CELL 18: Country / scenario selection ─────────────────────
# NOTE: this cell calls Python's input(), which is BY DESIGN — it pauses
# and waits for you to type a country name/number into the box under
# this cell's output, then press Enter. That's not a hang: if you use
# "Run all" and then walk away, it will sit here indefinitely waiting
# for you, which looks identical to a genuine freeze. Either run this
# cell by itself and answer the prompt, or skip the prompt entirely by
# setting CRISIS_COUNTRY_OVERRIDE below to a country string from your
# data (e.g. "China") before running — leave it as None to be prompted.
CRISIS_COUNTRY_OVERRIDE = None   # e.g. "China" — skips the input() prompt
...
def run_agent(scenario: str, crisis_country: str):
    ...
    supplier_names = [s["name"] for s in enriched]             # already unique
    ...
    # numbers that will be plotted, instead of trusting the LLM's
    # separately self-reported (and potentially inconsistent) field. ──
    ...
    # If the user picked a specific country from the data-driven list in
    # Cell 18, that EXACT string (guaranteed to exist verbatim in your
    # supply_chain.csv / ownership.csv country columns) is what every
    # downstream filter must match against — never the LLM's own
    # paraphrase of it ("United States" vs. your data's "USA", etc).
    # Only for a genuinely free-text/"Multiple" scenario do we fall back
    # to whatever the LLM inferred, and even then it gets passed through
    # normalize_country() wherever it's used for matching.
...
# ── CELL 17: Plotly dashboard (run this AFTER the script above) ───
# This is your "second cell" — it only reads the `analysis` /
# `ownership_data` variables produced by Cell 19 above. As long as you
# run the two cells in the same kernel/session, they will always be in
# sync now, because the header numbers below come from the exact same
# `analysis["portfolio_risk_score"]` / `analysis["overall_risk_level"]`
# fields that were recomputed deterministically at the end of Cell 19 —
# not from a separate LLM self-report.
...
# notebook renderer. Force it explicitly; we also fall back to an
# embedded static HTML render further below so the figure is GUARANTEED
# to appear even if the live renderer misbehaves.
...
def render_dashboard(analysis: dict, ownership_data: list):
    ...
    # `analysis["crisis_countries"]` is always the EXACT string the user
    # picked from the data-driven country list (see run_agent), or the LLM's
    # free-text guess only when no specific country was selected. Matching
    # goes through normalize_country() on BOTH sides, so "USA" in your data
    # and "United States" from the LLM/scenario text resolve to the same key.
    #
    # A supplier is considered "in the crisis zone" if ANY of three
    # independent, fully general exposure paths apply — none of this is
    # hardcoded to any single country; the same logic runs for whichever
    # country you type in:
    #
    #   (1) DIRECT            — the supplier itself is located in the crisis country
    #   (2) TIER-2 DEPENDENCY — a Tier-1 supplier whose Tier-2 supplier sits in the
    #                           crisis country (a disruption at Tier-2 propagates
    #                           UP to the Tier-1 that depends on it)
    #   (3) OWNERSHIP-LINKED  — ANY supplier with an owner based in the crisis
    #                           country holding >= OWNERSHIP_DEPTH_PCT% stake
    #                           (e.g. a Norway-based owner with a qualifying
    #                           stake in a French Tier-2 like Michelin)
    ...
    # result means exactly what it says — no supplier in your data is tied
    # to that crisis country via any of the three paths above — and the
    # chart renders an explicit "no match" annotation instead of silently
    # substituting the entire supplier list.
    ...
    #              → capped at 100
    # Base Weight = the supplier's deterministic baseline_country_risk.
    # The result is blended with the LLM/baseline score by taking whichever
    # is HIGHER — a crisis (direct, propagated, or ownership-linked) can
    # only ever raise a supplier's standing in the portfolio, never quietly
    # lower it relative to what was already known about it.
    ...
    # Full Supplier Risk Register and the portfolio header agree with the
    # bar chart instead of showing stale, pre-boost numbers.
    ...
    # ── v15 FIX #1 ────────────────────────────────────────────────
    # `crisis_supplier_names` is the set of Tier-1/Tier-2 suppliers that
    # are ACTUALLY exposed to the selected crisis country (Direct,
    # Tier-2 Dependency, or Ownership-Linked — computed above). The
    # Alternate Suppliers table on the dashboard must be built from
    # `crisis_alts` — alternates that replace one of THOSE suppliers —
    # never from the full `alternates` list, which covers every
    # supplier in the entire portfolio regardless of crisis relevance.
    ...
    # purely via the Ownership-Linked exposure path too (e.g. a Michelin-type
    # Tier-2 whose owner sits in the crisis country, even though Michelin
    # itself is headquartered elsewhere).
    ...
    # dedupes by name), but if this function is ever called with a list
    # built another way, guard here too so the table can never double-print
    # the same supplier's ownership rows.
    seen_supplier = set()
    ...
    # ── v15 FIX #2 ────────────────────────────────────────────────
    # `soe_flagged` now counts SOE/state-linked owners ONLY among
    # suppliers actually exposed to the selected crisis country
    # (`crisis_ownership`), not the full portfolio (`ownership_data`).
    # Previously this was computed from the full, crisis-independent
    # `ownership_data` list, so the "SOE Flags" KPI showed the SAME
    # portfolio-wide number (e.g. "3") no matter which crisis country
    # was selected. It will now correctly read 0 when no crisis-zone
    # supplier has a qualifying state-linked owner.
    ...
    # (Intentionally NOT crisis-scoped — shows EVERY supplier in the
    # portfolio by design, so the Indian Army can see the full picture
    # alongside the crisis-specific views. Do not filter this table.)
    def _status(v):
    ...
    # Only owners of suppliers that are exposed to the crisis country are
    # shown; this is what "Ownership Analysis should mention just those
    # owners related to the crisis country" means concretely. The owner
    # themselves may be in any country — what scopes the table is the
    # supplier's exposure, not the owner's country. (Owner country is
    # shown in the "Owner Country" column.)
    own_rows = []
    for o in crisis_ownership:
        for ow in o["owners"]:
    ...
    # v15 FIX #1: built from `crisis_alts` (alternates that replace a
    # crisis-zone Tier-1/Tier-2 supplier), NOT the full `alternates` list
    ...
    # "no bar graphs" issue when Colab's interactive widget renderer is
    # in a bad state for that session.
